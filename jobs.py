...
def assign_worker_qualification(qual_type_id, worker_id, value, send_notification):
    job = q.enqueue_call(
        func=mturk.assign_qualification, args=(qual_type_id, worker_id, value, send_notification),
        result_ttl=5000
    )
    return job
...
def schedule_check_for_start_assessment(user_id):
    time = datetime.utcnow() + timedelta(hours=1)
    schedule = scheduler.enqueue_at(time, check_for_start_assessment, user_id)
    return schedule
